...
public class DealStructureService
{
	...
	public DealStructure updateDealStructure(CreateDealStructureDTO payload) throws Exception
	{
		validatePayload(payload);
		List<DealStructure> dsList = dealStructureRepo.getDealStructureByLeadID(payload.getLeadId());
		DealStructure ds = null;
		if (dsList.size() > 1)
			throw new Exception("More than one Deal Structure record found. Please contact Administrator");
		else if (dsList.size() == 1)
			ds = dsList.get(1);
		else
			ds = new DealStructure();
	...
	private void validatePayload(CreateDealStructureDTO payload) throws Exception
	{
		if (payload.getLeadId() == null)
			throw new Exception("Lead ID is mandatory field. Please provide LeadID");

		if (!clRepo.existsById(payload.getLeadId()))
			throw new Exception("Lead not found with ID - " + payload.getLeadId());
