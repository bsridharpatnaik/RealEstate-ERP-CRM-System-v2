		...
		if (stagnantStatus != null && stagnantStatus.size() > 0)
		{
			...
			for (String str : stagnantStatus)
			{
				...
				if (str.equalsIgnoreCase("Green"))
					internalSpec = specbldr.specOrCondition(internalSpec,
							specbldr.whereChildFieldIntBetween(LeadActivity_.LEAD, Lead_.STAGNANT_DAYS_COUNT, 11, 20));
				if (str.equalsIgnoreCase("Orange"))
					internalSpec = specbldr.specOrCondition(internalSpec,
							specbldr.whereChildFieldIntBetween(LeadActivity_.LEAD, Lead_.STAGNANT_DAYS_COUNT, 21, 30));
				if (str.equalsIgnoreCase("Red"))
					internalSpec = specbldr.specOrCondition(internalSpec, specbldr.whereChildFieldIntBetween(
							LeadActivity_.LEAD, Lead_.STAGNANT_DAYS_COUNT, 31, Integer.MAX_VALUE));
			}
