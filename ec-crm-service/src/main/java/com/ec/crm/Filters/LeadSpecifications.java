...
public final class LeadSpecifications 
{
	...
	public static Specification<Lead> getSpecification(FilterDataList filterDataList) throws ParseException
	{
		...
		if(primarymobile != null && primarymobile.size()>0)
			finalSpec = specbldr.specAndCondition(finalSpec,specbldr.whereDirectFieldContains(Lead_.PRIMARY_MOBILE,primarymobile));
		...
		if(leadStatus != null && leadStatus.size()>0)
			finalSpec = specbldr.specAndCondition(finalSpec,specbldr.whereDirectFieldContains(Lead_.STATUS,leadStatus));
