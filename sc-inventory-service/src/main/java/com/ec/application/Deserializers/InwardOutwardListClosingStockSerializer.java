...
public class InwardOutwardListClosingStockSerializer extends StdSerializer<InwardOutwardList>
{
	...
	public void serialize(InwardOutwardList value, JsonGenerator gen, SerializerProvider provider) throws IOException
	{
		...
		gen.writeStringField(InwardOutwardList_.CREATION_DATE, df.format(value.getCreationDate()));
		gen.writeStringField(InwardOutwardList_.LAST_MODIFIED_DATE, df.format(value.getLastModifiedDate()));
		gen.writeStringField(InwardOutwardList_.LAST_MODIFIED_BY, value.getLastModifiedBy());
